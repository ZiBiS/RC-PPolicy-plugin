 ...
 * user to change password immediatly after login and show the proper message.
 *
 * @version @package_version@
 * @license GNU GPLv3+
 * @author Zbigniew Szmyd (zbigniew.szmyd@linseco.pl)
 * @website http://www.linseco.pl
 */
class ppolicy_checker extends rcube_plugin {
	...
	function init() {
		...
		$this->load_config ();
		$this->log_file = 'ppolicy_log.txt';
		$this->debug    = $this->rc->config->get ( 'ppolicy_debug' );
		
		$this->uri = $this->rc->config->get ( 'ppolicy_ldap_uri' );
		
		$this->basedn = $this->rc->config->get ( 'ppolicy_basedn' );
		$this->ppolicy_policies_basedn = $this->rc->config->get ( 'ppolicy_policies_base_dn' );
		$this->default_policy = $this->rc->config->get ( 'ppolicy_default_policy' );
		
		$this->ldap_config = array (
				'binddn'  => $this->rc->config->get ( 'ppolicy_binddn' ),
				'bindpw'  => $this->rc->config->get ( 'ppolicy_bindpw' ),
				'basedn'  => $this->basedn,
				'version' => 3 
		);
